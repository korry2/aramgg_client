...
- 主界面「界面与数据语言」支持 `zh-CN`、`en-US` 和 `zh-TW`。选择成功后，主窗口、英雄详情、席位推荐、海克斯浮窗、右侧推荐列表和赛后海报会统一切换文案与数据。
- 语言切换是先准备、后提交的事务：只有本地或数据 API 存在完整且精确匹配的目标语言数据集时，才会广播 `locale-changed` 并切换 Vue i18n；失败时界面和数据都保留原语言。
