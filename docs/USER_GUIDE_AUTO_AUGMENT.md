...
主界面「界面与数据语言」可选择简体中文、English 或繁體中文。切换成功后，主窗口、英雄详情、席位推荐、海克斯顶部浮窗、右侧推荐列表和赛后海报会立即使用目标语言，并重新加载同语言英雄、海克斯和装备数据。

应用会先确认目标语言的完整数据集可用，再提交语言变更。准备期间只有语言选择框显示局部进度，其余主界面保持可操作；提交后的远端版本信息在后台刷新，不会继续占用切换 loading。准备失败时选择器会恢复，所有窗口和数据继续使用原语言，不会出现界面已切换但数据仍为旧语言的半切换状态。新窗口启动时会先读取当前语言再挂载界面。
